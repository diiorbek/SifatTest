...
from aiogram.types import Message, CallbackQuery, FSInputFile, InlineKeyboardButton, ContentType
...
from other.questions import add_question, find_result_index, new_mistakes, show_mistakes, back_txt
from buttons import buttons
from states import Form, Adverts, MessageAdmin, AdminStates
from other.start import test, all_answers, tests, users_rating
...
async def start_the_test(message: Message):
    ...
    if message.from_user.id not in list(users.keys()):
        ...
        options = []
        for i in users[message.from_user.id]['testing'][0][1:-1]:
            options.append(str(i))

        await bot.send_poll(chat_id=message.from_user.id, 
                            question=f"1. {users[message.from_user.id]['testing'][0][0]}",
                            options=options,
                            type="quiz",
                            correct_option_id=options.index(users[message.from_user.id]['testing'][0][-1]),
                            is_anonymous=False,
                            open_period=5)
        print(len(users[message.from_user.id]['testing']))
        for sdvbsdfg in range(len(users[message.from_user.id]['testing'])):
            try:
                await asyncio.sleep(5)
                if users[message.from_user.id]['question'] == 1 or users[message.from_user.id]['answers'][-1] == "not_answered":
                    options = users[message.from_user.id]['testing'][users[message.from_user.id]["question"]][1:-1]

                    correct_answer = users[message.from_user.id]['testing'][users[message.from_user.id]['question']][-1]
                    correct_option_id = options.index(correct_answer)

                    if users[message.from_user.id]["exam"] == False:
                        await bot.send_poll(
                            chat_id=message.from_user.id,
                            question=f"{users[message.from_user.id]['question'] + 1}. {users[message.from_user.id]['testing'][users[message.from_user.id]['question']][0]}",
                            options=options,
                            type="quiz",
                    ...
                    users[message.from_user.id]["question"] += 1
                    users[message.from_user.id]["answers"].append("not_answered")
            except IndexError:
                nat = 0
                questions_count = users[message.from_user.id]["question"]
                ...
                global txt
                txt = f"{message.from_user.full_name}, siz testni tugatdingiz!\nSizning natijangiz: {users[message.from_user.id]['correct']}/{users[message.from_user.id]['question']} | {int(percent)}%\n{tabrik}"
                ...
                if percent < 100 and percent >= 50:
                    await bot.send_message(chat_id=message.from_user.id, text=txt, reply_markup=buttons.test_end)

                if percent < 50:
                    voice = FSInputFile("voice.ogg")
                    await bot.send_audio(chat_id=message.from_user.id, audio=voice, caption=txt, reply_markup=buttons.test_end)

                save_data(full_name=message.from_user.full_name, user_id=message.from_user.id, result=users[message.from_user.id]["correct"], max_result=users[message.from_user.id]["question"], kind=users[message.from_user.id]["kind"], is_exam=users[message.from_user.id]["exam"])

                for admin in ADMINS:
                    if users[message.from_user.id]['exam'] == False:
                        await bot.send_message(chat_id=admin, text=f"{message.from_user.full_name} testni tugatdi!\n{users[message.from_user.id]['kind']} bo'yicha {users[message.from_user.id]['correct']}/{users[message.from_user.id]['question']} | {int(percent)}% yechdi.")
                    if users[message.from_user.id]['exam'] == True:
                        await bot.send_message(chat_id=admin, text=f"@{message.from_user.username} imtihon testini tugatdi!\nImtihonni {users[message.from_user.id]['correct']}/{users[message.from_user.id]['question']} | {int(percent)}% yechdi.")

                if users[message.from_user.id]['exam'] == True:
                    exam_txt[0] += f"• 👤{message.from_user.full_name} - {int(percent)}%\n"
                    print(exam_txt[0])
...
async def poll_answer_handler(poll_answer: PollAnswer):
    try:
        options = users[poll_answer.user.id]['testing'][users[poll_answer.user.id]["question"]][1:-1]
        
        correct_answer = users[poll_answer.user.id]['testing'][users[poll_answer.user.id]['question']][-1]
        correct_option_id = options.index(correct_answer)

        if users[poll_answer.user.id]["exam"] == False:
            await bot.send_poll(
                chat_id=poll_answer.user.id,
                question=f"{users[poll_answer.user.id]['question'] + 1}. {users[poll_answer.user.id]['testing'][users[poll_answer.user.id]['question']][0]}",
                options=options,
                type="quiz",
                correct_option_id=correct_option_id,
                is_anonymous=False,
        ...
        users[poll_answer.user.id]["question"] += 1
        users[poll_answer.user.id]["answers"].append(poll_answer.option_ids[0])
        
        
    except IndexError:
        users[poll_answer.user.id]["answers"].append(poll_answer.option_ids[0])
        
        for i in range(0, users[poll_answer.user.id]['question']):
            if users[poll_answer.user.id]["answers"][i] == users[poll_answer.user.id]['testing'][i][1:-1].index(users[poll_answer.user.id]['testing'][i][-1]):
                users[poll_answer.user.id]["correct"] += 1
            else:
                new_mistakes(
    user_id=str(poll_answer.user.id), 
    question=users[poll_answer.user.id]['testing'][i][0], 
    user_answer=users[poll_answer.user.id]['testing'][i][1:-1][users[poll_answer.user.id]["answers"][i]], 
    correct_answer=users[poll_answer.user.id]['testing'][i][-1], 
    test=(int(len_tests(poll_answer.user.id)) + 1)
)
        
        nat = users[poll_answer.user.id]["correct"]
        questions_count = users[poll_answer.user.id]["question"]
        ...
        global txt
        txt = f"{poll_answer.user.full_name}, siz testni tugatdingiz!\nSizning natijangiz: {users[poll_answer.user.id]['correct']}/{users[poll_answer.user.id]['question']} | {int(percent)}%\n{tabrik}"
        
        if percent == 100:
            await bot.send_message(chat_id=poll_answer.user.id, text=txt)

            
        if percent < 100 and percent >= 50:
            await bot.send_message(chat_id=poll_answer.user.id, text=txt, reply_markup=buttons.test_end)
        
        if percent < 50:
            voice = FSInputFile("voice.ogg")
            await bot.send_audio(chat_id=poll_answer.user.id, audio=voice, caption=txt, reply_markup=buttons.test_end)
        
        save_data(full_name=poll_answer.user.full_name, user_id=poll_answer.user.id, result=users[poll_answer.user.id]["correct"], max_result=users[poll_answer.user.id]["question"], kind=users[poll_answer.user.id]["kind"], is_exam=users[poll_answer.user.id]["exam"])
            
        for admin in ADMINS:
            if users[poll_answer.user.id]['exam'] == False:
                await bot.send_message(chat_id=admin, text=f"{poll_answer.user.full_name} testni tugatdi!\n{users[poll_answer.user.id]['kind']} bo'yicha {users[poll_answer.user.id]['correct']}/{users[poll_answer.user.id]['question']} | {int(percent)}% yechdi.")
            if users[poll_answer.user.id]['exam'] == True:
                await bot.send_message(chat_id=admin, text=f"@{poll_answer.user.username} imtihon testini tugatdi!\nImtihonni {users[poll_answer.user.id]['correct']}/{users[poll_answer.user.id]['question']} | {int(percent)}% yechdi.")
          
        if users[poll_answer.user.id]['exam'] == True:
            exam_txt[0] += f"• 👤{poll_answer.user.full_name} - {int(percent)}%\n"
            print(exam_txt[0])
        del users[poll_answer.user.id]
        print(users)
        # await bot.send_message(chat_id=poll_answer.user.id, text=f"")
...
async def test_1(callback: CallbackQuery, state: FSMContext):
        if callback.data == "mistakes":
            ...
            text_for_back = back_txt(callback.from_user.full_name, callback.from_user.id)
            await bot.delete_message(chat_id=callback.from_user.id, message_id=callback.message.message_id)
            await bot.send_message(chat_id=callback.from_user.id, text=text_for_back, reply_markup=buttons.test_end)
            ...
            options = []
            for i in users[callback.from_user.id]['testing'][0][1:-1]:
                options.append(str(i))

            await bot.send_poll(chat_id=callback.from_user.id, 
                                question=f"1. {users[callback.from_user.id]['testing'][0][0]}",
                                options=options,
                                type="quiz",
                                correct_option_id=options.index(users[callback.from_user.id]['testing'][0][-1]),
                                is_anonymous=False)
...
async def kind_state(message: Message, state: FSMContext):
    ...
    try:
        tests.append([question, a_var, b_var, c_var, d_var, correct_answer])
